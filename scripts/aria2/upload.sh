...
CHECK_RCLONE() {
    if [[ ${RCLONE_UPLOAD_PROVIDER} == "" ]]; then
...
DEFINITION_PATH() {
    ...
    if [[ -f "${TASK_PATH}" ]]; then
       REMOTE_PATH="${RCLONE_UPLOAD_PROVIDER}:${RCLONE_UPLOAD_REMOTE_PATH}${DEST_PATH_SUFFIX%/*}"
    else
        REMOTE_PATH="${RCLONE_UPLOAD_PROVIDER}:${RCLONE_UPLOAD_REMOTE_PATH}${DEST_PATH_SUFFIX}"
    fi
}
...
CLEAN_MKV_FILES() {
    ...
    if echo "$LOCAL_PATH" | grep -q '\.mkv$'; then
     mkclean "$LOCAL_PATH" "${LOCAL_PATH}.clean" > /dev/null 2>&1
     mv "${LOCAL_PATH}.clean" "$LOCAL_PATH"
    fi
}
